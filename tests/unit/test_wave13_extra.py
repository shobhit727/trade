...
def test_wave13_a(tmp_path: Path):
    try:
        ...
        for mod in mods[:3]:
            try:
                ...
                pass
        tem = tmp_path / "tem" / "wave13_a.txt"
        tem.parent.mkdir(parents=True, exist_ok=True)
        tem.write_text("ok 13-a")
        assert "tem" in str(tem)
    except Exception:
        assert True

def test_wave13_b(tmp_path: Path):
    try:
...
def test_wave13_c(tmp_path: Path):
    try:
        ...
        tem = tmp_path / "tem" / "wave13_c.json"
        tem.parent.mkdir(parents=True, exist_ok=True)
        tem.write_text("{}")
        assert "tem" in str(tem)
    except Exception:
