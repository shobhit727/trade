"""Tests for catalog strategy dispersion."""

from __future__ import annotations

import math

from cryptobot.strategies.catalog.dispersion import DispersionStrategy


def _series(up: bool):
    s = DispersionStrategy()
    out = []
    for i in range(300):
...
def test_dispersion_long_signal():
    assert any(o.side.value == "BUY" for o in _series(True))


def test_dispersion_short_signal():
