...
def run_one(bars, name, args):
    try:
        strat = make_strategy(name)
    except Exception as e:
        return {"error": f"make: {e}"[:40]}
    result = asyncio.run(run_backtest(bars, strat, collect_trades=False, risk_fraction=0.01))
    ...
    v = run_validation(rets, n_splits=5, n_permutations=args.perms, n_trials=50)
    return {
...
CATALOG_NAMES = sorted(
    {(Path(p).stem) for p in Path("src/cryptobot/strategies/catalog").glob("*.py") if p.stem != "__init__"}
)


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("path", nargs="?", default="/tmp/opencode/btcusdt_1h.json")
    ap.add_argument("--sample", type=int, default=0, help="use last N bars (0 = all)")
    ap.add_argument("--perms", type=int, default=30)
    args = ap.parse_args()
    bars = load_bars(args.path)
    if args.sample > 0 and len(bars) > args.sample:
        bars = bars[-args.sample :]
    print(
        f"Loaded {len(bars)} real BTCUSDT 1h bars ({bars[0].timestamp.date()} → {bars[-1].timestamp.date()})"
    )
    ...
    for name in CATALOG_NAMES:
        ...
        print(
            f"{name:<22} {r['ret']:+8.3f} {r['wf']:+8.2f} {r['ds']:+6.2f} {r['mc_p']:>6.3f} {str(r['passed']):>5} {r['trades']:>6d}"
        )
    ...
    if passed:
        ...
        for name, ret, wf, ds, mc_p, _ok, trades, _ in sorted(passed, key=lambda r: r[1], reverse=True):
            print(
                f"  {name:<22} ret={ret * 100:+6.2f}%  wf={wf:+.2f}  ds={ds:+.2f}  mc_p={mc_p:.3f}  trades={trades}"
            )
    print(
