...
| ID | File | Bug | Risk |
|----|------|------|------|
| B024 | `core/state.py` | If `sqlite3` unavailable, persistence silently no-ops. State diverges across restarts. | Lose state. |
...
| B040 | `execution/engine.py` | `submit_order` publishes `OrderEvent` after risk rejection, but `type` is still `OrderType.MARKET` (the order type), not `EventType.ORDER_REJECTED`. Mixed semantics. | Subscriber confusion. |
| B041 | `monitoring/alerting.py` | Multiple channels all share `init_alerting` resource; no teardown error path. | Resource leak. |
...
| B044 | `market_data/manager.py` | `BinanceWSClient` builds stream URL from `settings.exchange.symbols` (empty by default). | Empty WS subscription. |
| B045 | `core/bus.py` | `EventBus.publish_batch` calls `_dispatch` per event; no transaction. | Bursty work. |
