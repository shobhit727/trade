...
impl PerformanceMetrics {
    pub fn calculate(returns: &[f64]) -> Self {
        ...
        let downside_std = if downside_returns.is_empty() {
            ...
            let downside_var = downside_returns.iter().map(|r| (r - downside_mean).powi(2)).sum::<f64>() / downside_returns.len() as f64;
            downside_var.sqrt()
        };
        ...
        Self {
            sharpe_ratio: sharpe * (252.0_f64).sqrt(), // Annualize
            sortino_ratio: sortino * (252.0_f64).sqrt(),
            max_drawdown: max_dd,
            profit_factor,
            ...
            avg_win: if !wins.is_empty() { wins.iter().sum::<f64>() / wins.len() as f64 } else { 0.0 },
            avg_loss: if !losses.is_empty() { losses.iter().sum::<f64>() / losses.len() as f64 } else { 0.0 },
        }
    }
}
